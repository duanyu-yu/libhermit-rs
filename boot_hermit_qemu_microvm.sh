...
sudo qemu-system-x86_64 -M microvm,x-option-roms=off,pit=off,pic=off,rtc=off \
        ...
        -m 64M \
        -serial stdio \
        -kernel ~/kernels/rusty-loader/target/x86_64-unknown-hermit-loader/debug/rusty-loader \
        -initrd ~/kernels/rusty-hermit/target/x86_64-unknown-hermit/debug/hello_world \
        -enable-kvm \
        -cpu host \
